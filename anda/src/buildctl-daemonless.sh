...
startBuildkitd() {
    ...
    if [ $(id -u) = 0 ]; then
        ...
        containerd_addr=unix://$XDG_RUNTIME_DIR/containerd/containerd.sock
        helper=$ROOTLESSKIT
    fi
    $helper $BUILDKITD $BUILDKITD_FLAGS --containerd-worker false --containerd-worker-addr=$containerd_addr --addr=$addr >$tmp/log 2>&1 &
    pid=$!
    echo $pid >$tmp/pid
    echo $addr >$tmp/addr
}
