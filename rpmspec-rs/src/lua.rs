...
impl RPMLuaParser {
	...
	pub(crate) fn run(rpmparser: &Arc<Mutex<SpecParser>>, script: &str) -> Result<String> {
		...
		lua.context(|ctx| -> rlua::Result<()> {
			...
			rpm.set("expand", ctx.create_function(move |ctx, arg: String| Self::expand(&p, ctx, &arg))?)?;
			let p = rpmparser.clone();
			rpm.set("define", ctx.create_function(move |ctx, arg| Self::define(&p, ctx, arg))?)?;
			let p = rpmparser.clone();
			rpm.set("undefine", ctx.create_function(move |ctx, arg| Self::undefine(&p, ctx, arg))?)?;
			let p = rpmparser.clone();
			rpm.set("isdefined", ctx.create_function(move |ctx, arg: String| Self::isdefined(&p, ctx, &arg))?)?;
			let p = rpmparser.clone();
			rpm.set("load", ctx.create_function(move |ctx, arg| Self::load(&p, ctx, arg))?)?;
