use crate::backend::{BuildDb, DatabaseEntity};
use crate::kubernetes::{get_full_logs, get_logs, watch_jobs, BuildStatusEvent, K8S};
        ...
        while let Some(job) = jobstream.next().await {
            match job {
                BuildStatusEvent::Running(id) => {
                    ...
                    let build = crate::backend::Build::get(Uuid::parse_str(&id).unwrap())
                        .await
                        .unwrap();
                    build
                        .update_status(BuildStatus::Running as i32)
                        .await
                        .unwrap();
                ...
                BuildStatusEvent::Succeeded(id) => {
                    ...
                    let build = crate::backend::Build::get(Uuid::parse_str(&id).unwrap())
                        ...
                        .unwrap();
                    build
                        .update_status(BuildStatus::Success as i32)
                        .await
                        .unwrap();
                    // update logs
                ...
                BuildStatusEvent::Failed(id) => {
                    ...
                    let build = crate::backend::Build::get(Uuid::parse_str(&id).unwrap())
                        ...
                        .unwrap();
                    build
                        .update_status(BuildStatus::Failure as i32)
                        .await
                        .unwrap();
                    // update logs
                    // let logs = full_logs(id).await.unwrap();
...
pub async fn get_pod_name_watch(id: String) -> Result<String> {
    ...
    let mut stream = pods
        .watch(&ListParams::default().labels(&filter), "0")
        .await?
        .boxed();
...
pub async fn full_logs(id: String) -> Result<String> {
    let pod_name = get_pod_name_existing(id).await?;
    Ok(get_full_logs(pod_name.clone()).await?)
}

pub async fn full_logs_db(id: String) -> Result<String> {
    // get build by id
    let build = crate::backend::Build::get(Uuid::parse_str(&id).unwrap())
        .await
        .unwrap();
...
pub async fn format_actual_stream(
    logstream: impl Stream<Item = Result<Bytes, kube::Error>>,
) -> Result<impl Stream<Item = String>> {
    // map this logstream into a trystream of strings
    Ok(logstream.map(|bytes| {
        String::from_utf8(bytes.unwrap().to_vec())
